    ...
    stages {
        ...
        stage('build') {
            steps {
              ...
              sh 'docker run -d -p 3000:3000 --mount type=bind,source=/home/gerardo/misc/sice-devops/log,target=/usr/src/app/log --name=sice-app sice-${GIT_BRANCH}:${BUILD_NUMBER}'
            }
        }
    }
    post {
